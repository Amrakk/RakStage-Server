...
export async function messageBrokerErrorHandler(err: any) {
    try {
        if (err instanceof BaseError) {
            if (err.statusCode >= 500) await errorLogger(err);

            return { status: err.statusCode, error: err.getResponseBody() };
        } else if (err instanceof MongooatError) return mongooatErrorHandler(err);
        else if (err instanceof MongoError) return mongoErrorHandler(err);
        ...
        await errorLogger(error).catch(console.error);
        console.error(err);
        return {
            code: RESPONSE_CODE.INTERNAL_SERVER_ERROR,
            message: RESPONSE_MESSAGE.INTERNAL_SERVER_ERROR,
...
function mongooatErrorHandler<T extends MongooatError>(err: T, res?: Response<IResponse>) {
    if (err instanceof ValidateError) {
        ...
        return res ? res.status(400).json(responseData) : responseData;
    }
